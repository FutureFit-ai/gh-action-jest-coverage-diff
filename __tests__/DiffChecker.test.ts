...
describe('DiffChecker', () => {
  ...
  describe('testing checkIfTestCoverageShouldIncrease', () => {
    it('returns false if minimum coverage was reached', () => {
      const codeCoverageOld = {
      ...
      const diffChecker = new DiffChecker(codeCoverageNew, codeCoverageOld)
      const isShouldIncrease = diffChecker.checkIfTestCoverageShouldIncrease(
        98,
        1
      )
      expect(isShouldIncrease).toBeFalsy()
    })
    it('returns false if minimum delta was reached', () => {
      const codeCoverageOld = {
        total: mock98CoverageFile
      }
      const codeCoverageNew = {
    ...
    it("returns true if minimum coverage or delta wasn't reached", () => {
      const mock10Coverage = {
        total: 100,
        covered: 10,
        skipped: 0,
        pct: 10
      }
      const mock10CoverageFile = {
        statements: mock10Coverage,
        branches: mock10Coverage,
        functions: mock10Coverage,
        lines: mock10Coverage
      }
      const codeCoverageOld = {
        total: mockEmptyCoverageFile
      }
      const codeCoverageNew = {
        total: mock10CoverageFile
      }
      const diffChecker = new DiffChecker(codeCoverageNew, codeCoverageOld)
      expect(diffChecker.checkIfTestCoverageShouldIncrease(12, 15)).toBeTruthy()
    })
  })
